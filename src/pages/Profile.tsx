...
function Profile() {
  ...
  const fetchUserPins = async () => {
    try {
      const response = await axios.get('http://localhost:8000/api/users/me/pins', {
  ...
  const handleSave = async (e: React.MouseEvent, pinId: number) => {
    ...
    try {
      if (savedPins.some(pin => pin.id === pinId)) {
        await axios.delete(`http://localhost:8000/api/pins/${pinId}/save`, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSnackbar({
          open: true,
        ...
        await axios.post(`http://localhost:8000/api/pins/${pinId}/save`, {}, {
          headers: { Authorization: `Bearer ${token}` }
        });
        setSnackbar({
          open: true,
